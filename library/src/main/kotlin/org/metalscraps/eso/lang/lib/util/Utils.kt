...
class Utils {
    ...
    companion object {
        private val ignoringFileNames = arrayOf("00_EsoUI_Client", "00_EsoUI_Pregame");
        ...
        */ ////////////////////////////////////////////////////
        ...
        public fun getPO(path: Path, opt: TextParseOptions): ArrayList<PO> {
            //val poMap = HashMap<String, PO>()
            val poList = ArrayList<PO>()
            ...
            AppConfig.POPattern.matcher(source).run {
                while (find()) {
                    ...
                    */
                    poList.add(parsePO(this, fileName, opt))
                }
            }
            return poList
        } // getPO

        fun getPOMap(fileList: List<Path>, options: TextParseOptions = TextParseOptions()): MutableMap<String, PO> {
            return getPOList(fileList, options).stream().collect(Collectors.toMap(
                    {
                        it.getID()
                    },
                    {
        ...
        fun getPOList(fileList: List<Path>, options: TextParseOptions = TextParseOptions()): List<PO> {
            return fileList.stream().parallel().map {
                if (it.fileName() in ignoringFileNames) return@map emptyList<PO>()
        ...
        */ ////////////////////////////////////////////////////
        ...
        fun listFiles(path: Path = vars.poDir, ext: String = "po"): MutableList<Path> {
            try {
                return Files.list(path).filter { x -> !Files.isDirectory(x) && x.ext() == ext }.collect(Collectors.toList())
            } catch (e: IOException) {
                e.printStackTrace()
            }
            return ArrayList()
        ...
        */ ////////////////////////////////////////////////////
        ...
        fun crc32(p: Path): Long {
            val crc = CRC32()
            try {
                if (Files.notExists(p) || Files.size(p) <= 0) return -1
            } catch (e: IOException) {
                e.printStackTrace()
            }
        ...
        */ ////////////////////////////////////////////////////
        ...
        fun processRun(baseDirectory: Path, command: String) {
            processRun(baseDirectory, command, ProcessBuilder.Redirect.INHERIT)
        }
